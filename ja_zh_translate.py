...
def openai_call(input_message):
    ...
    cur_messages = []
    cur_messages.append({"role": "system", "content": '你是一个同声传译助手，把输入的中文翻译为日文，日文翻译为中文，注意，只输出翻译的内容.'})
...
def generate_tts(text):
    ...
    agent = "zh-CN-XiaoyiNeural"
    if result[0][0]=="zh":
        agent = "zh-CN-XiaoyiNeural"
    elif result[0][0]=="ja":
        agent = "ja-JP-NanamiNeural"
    elif result[0][0]=="en":
        agent = "en-US-EmmaNeural"
    print("detected language is : {}".format(result[0][0]))
    communicate = edge_tts.Communicate(text, agent)
